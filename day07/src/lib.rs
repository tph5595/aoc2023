...
impl Hand {
    fn calc_value(cards: &Vec<u32>, jokes: usize) -> u32{
        ...
        if k.iter().any(|i| **i == (4-jokes) as u32 ) || jokes == 4{
            return 6;
        }
        ...
        if k.iter().any(|i| **i == (3-jokes) as u32) || jokes == 3{
            return 4;
        }
...
impl FromStr for Hand{
    ...
    fn from_str(input: &str) -> Result<Hand, Self::Err> {
        ...
        let cards: Vec<u32>= data.next().unwrap().chars().map(|c|{
            match c {
                'A'=> 14,
                'K'=> 13,
                'Q'=> 12,
        ...
        let jokes: usize = cards.iter()
            .filter(|i| **i == 1)
            .map(|i| *i as usize)
            .sum();
        let value = Hand::calc_value(&cards, jokes);

        Ok(Hand{ cards, bet, value, jokes})
    }
}
